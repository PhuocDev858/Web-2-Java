...
import { ImageUploader, type ImageUploaderHandle } from '@/components/common/ImageUploader'
...
interface ProductFormModalProps {
  ...
  onSubmitWithFiles?: (product: Partial<Product>, imageUploaderRef: React.RefObject<ImageUploaderHandle>) => Promise<void>
  isLoading?: boolean
}
...
export const ProductFormModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Dùng onSubmitWithFiles nếu có, để xử lý upload files sau
      if (onSubmitWithFiles) {
        await onSubmitWithFiles(data, imageUploaderRef)
      } else {
        await onSubmit(data)
      }
    } catch (err) {
        ...
        <fieldset disabled={isLoading} className="space-y-4">
          ...
          <div>
            ...
            <ImageUploader
              ...
              maxImages={5}
              onUpload={(url) => console.log('File selected:', url)}
            />
          </div>
        </fieldset>
      </form>
